...
// node_modules/.pnpm/axios@1.7.9/node_modules/axios/dist/node/axios.cjs
  ...
  "node_modules/.pnpm/axios@1.7.9/node_modules/axios/dist/node/axios.cjs"(
    ...
    var textEncoder =
      typeof TextEncoder === "function"
        ? new TextEncoder()
        : new util__default["default"].TextEncoder();
    var CRLF = "\r\n";
    ...
    function setProxy(options, configProxy, location) {
      ...
      if (!proxy && proxy !== false) {
        const proxyUrl =
          proxyFromEnv__default["default"].getProxyForUrl(location);
        if (proxyUrl) {
...
function faucetSUI() {
  ...
  const faucet = async (address, count) => {
    ...
    try {
      ...
      const success_message = `\u2705 [${address}] - from ${ip} - ${/* @__PURE__ */ new Date().toLocaleString()}`;
      await sendMessage(success_message);
      console.log(success_message);
      return;
    } catch (error) {
      if (count <= 0) {
        await sendMessage(`\u{1F525} ${address}] - from ${ip} - failed.`);
        return;
      }
      const error_message = `\u2757 [${address}] - from ${ip} - ${/* @__PURE__ */ new Date().toLocaleString()}`;
      await sendMessage(error_message);
      console.error(error_message);
      await new Promise((resolve) => setTimeout(resolve, 1e3 * 60 * 5));
